...
### Apply用のIAM Role ### 
data "aws_iam_policy_document" "github_oidc_assume_role_policy_apply" {
  statement {
    actions = ["sts:AssumeRoleWithWebIdentity"]
    effect  = "Allow"
...
resource "aws_iam_role" "github_actions_terraform_apply_role" {
  name               = "GitHubActions_Terraform_AWS_terraform_apply"
  assume_role_policy = data.aws_iam_policy_document.github_oidc_assume_role_policy_apply.json
}

# Administrator policyをroleにアタッチ
resource "aws_iam_role_policy_attachment" "github_actions_terraform_apply_policy_attachment" {
  role       = aws_iam_role.github_actions_terraform_apply_role.name
